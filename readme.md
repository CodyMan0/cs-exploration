...
> 알고리즘·자료구조 핵심을 기초 복잡도부터 그래프 심화까지 체계적으로 다루는 커리큘럼.

## Chapter 1. 알고리즘 기초 & 복잡도

| 주제 | 핵심 키워드 | 상태 |
|------|-------------|------|
| 알고리즘과 효율성 | 좋은 알고리즘의 조건, 정확성 검증 | ✅ CLEAR |
| 빅오 표기법 | O(1)·O(N)·O(N²)·O(logN)·O(NlogN) | ✅ CLEAR |
| 선형 자료구조 | 배열, 스택, 큐, 연결 리스트 | ✅ CLEAR |
| 해시 테이블 | 해시 함수, 해시 맵, O(1) 탐색 | ✅ CLEAR |

## Chapter 2. 재귀 · 탐색 · 복잡도 이론

| 주제 | 핵심 키워드 | 상태 |
|------|-------------|------|
| 재귀 | 호출 과정, 장단점, 꼬리 재귀 | ⬜ |
| 주먹구구식(브루트포스) | 완전 탐색, 기하급수적 복잡도 | ⬜ |
| P vs NP | P, NP, NP-완전, NP-난해 | ⬜ |
| 이진 탐색 | 정렬 전제, O(logN), 회전 배열 응용 | ⬜ |
...
## Chapter 4. 해시 & 데이터 무결성

| 주제 | 핵심 키워드 | 상태 |
|------|-------------|------|
| 해시 함수 | 균일성, 지역 민감 해시, 효율성 | ⬜ |
| 무결성 검사 | 체크섬, CRC, 패리티 비트 | ⬜ |
| 암호학적 해시 | 생일 문제, 비밀번호 해싱·솔트 | ⬜ |
...
| 주제 | 핵심 키워드 | 상태 |
|------|-------------|------|
| 트리와 BST | 이진 탐색 트리, 삽입/삭제 | ⬜ |
| 트리 순회 | 전위·중위·후위 | ⬜ |
| 균형 트리 | 레드-블랙 트리 | ⬜ |

## Chapter 7. 트라이 & 공간분할 트리
...
|------|-------------|------|
| DFS / BFS | 깊이 우선, 너비 우선 | ⬜ |
| 미니맥스 | 게임 트리, 깊이 제한, 평가 함수 | ⬜ |

## Chapter 9. 동적 계획법 & 그리디

| 주제 | 핵심 키워드 | 상태 |
|------|-------------|------|
| 동적 계획법 | 메모이제이션, 타뷸레이션, 배낭 문제 | ⬜ |
| 그리디 | 탐욕 선택, 인터벌 스케줄링 | ⬜ |
| 압축 | 허프만 코딩, 양자화 | ⬜ |

## Chapter 10. 그래프 I — 표현 & 순회

| 주제 | 핵심 키워드 | 상태 |
|------|-------------|------|
| 그래프 표현 | 인접 행렬, 인접 리스트 | ⬜ |
| 그래프 DFS & 위상 정렬 | 방향 그래프, 위상 순서 | ⬜ |
| 강한 결합 요소 | SCC, 코사라주 알고리즘 | ⬜ |

## Chapter 11. 그래프 II — 최단 경로

| 주제 | 핵심 키워드 | 상태 |
|------|-------------|------|
| BFS 최단 경로 & 다익스트라 | 우선순위 큐, 음의 가중치 | ⬜ |
| A* | 휴리스틱, h(n) | ⬜ |
| 플로이드-워셜 | 모든 쌍 최단 경로 | ⬜ |

## Chapter 12. 그래프 III — 심화

| 주제 | 핵심 키워드 | 상태 |
|------|-------------|------|
| 최소 신장 트리 | 크러스컬, 서로소 집합 | ⬜ |
| 외판원 문제 | TSP, 2-근사 | ⬜ |
| 최대 유량 | 흐름 네트워크, 에드몬드-카프 | ⬜ |

## Chapter 13. 기타 알고리즘

| 주제 | 핵심 키워드 | 상태 |
|------|-------------|------|
| 상태 기계 & 정규식 | 유한 상태 기계, 패턴 매칭 | ⬜ |
| 패턴 인식 | KNN, 정규화 | ⬜ |
